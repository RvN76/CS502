...
void test1b(void) {
	...
	CREATE_PROCESS("test1b_a", test1x, ILLEGAL_PRIORITY, &Z502_REG1, &Z502_REG9);
...
void test1h(void) {
	...
	// Make legal targets
	CREATE_PROCESS("test1h_a", test1x, NORMAL_PRIORITY, &Z502_REG3, &Z502_REG9);
	CREATE_PROCESS("test1h_b", test1x, NORMAL_PRIORITY, &Z502_REG4, &Z502_REG9);
	CREATE_PROCESS("test1h_c", test1x, NORMAL_PRIORITY, &Z502_REG5, &Z502_REG9);
...
void test1i(void) {
	...
	// Make a legal target
	CREATE_PROCESS("test1i_a", test1x, NORMAL_PRIORITY, &Z502_REG3, &Z502_REG9);
	...
	while (Z502_REG9 == ERR_SUCCESS) {
		td->target_pid = Z502_REG3;
		td->send_length = LEGAL_MESSAGE_LENGTH;
		SEND_MESSAGE(td->target_pid, "Legal Test1i Message", td->send_length,
				&Z502_REG9);
		td->loop_count++;
	}  // End of while
	...
	for (Iterations = 0; Iterations < TEST2C_LOOPS; Iterations++) {
		...
		if ((data_read->int_data[0] != data_written->int_data[0])
			...
			printf("ERROR in Test 2c, Pid = %ld. Disk = %ld\n", Z502_REG4,
					disk_id);
			printf("Written:  %d,  %d,  %d,  %d\n", data_written->int_data[0],
					data_written->int_data[1], data_written->int_data[2],
					data_written->int_data[3]);
			printf("Read:     %d,  %d,  %d,  %d\n", data_read->int_data[0],
					data_read->int_data[1], data_read->int_data[2],
					data_read->int_data[3]);
		} else if ((Iterations % DISPLAY_GRANULARITY2c) == 0) {
			printf(
					"TEST2C: PID = %ld: SUCCESS READING  disk_id =%ld, sector = %ld\n",
